...
export async function addReputationForReaction(input: {
	...
	const rateLimitCheck = rateLimitService.checkLimits(input.guildId, input.reactor.id, input.recipient.id);
	...
	if (!rateLimitCheck.allowed) {
		...
		console.debug(
			`Rate limit exceeded: ${rateLimitCheck.reason} for user ${input.reactor.id} in guild ${input.guildId}`
		);
		return {
			success: false,
			error,
